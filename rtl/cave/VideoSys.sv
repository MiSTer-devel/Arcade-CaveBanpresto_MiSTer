...
module VideoSys(
  ...
  wire        wideCompatibilityClockEnable;
  wire        wideCompatibilityDisplayEnable;
  wire [8:0]  wideCompatibilityPosX;
  wire [8:0]  wideCompatibilityPosY;
  wire        wideCompatibilityHSync;
  wire        wideCompatibilityVSync;
  wire        wideCompatibilityHBlank;
  wire        wideCompatibilityVBlank;
  ...
  always @(posedge io_videoClock) begin
    if (io_videoReset) begin
      ...
      if (wideVSync | wideCompatibilityVSync) begin
        wideOffsetX <= io_options_offset_x;
        wideOffsetY <= io_options_offset_y;
      end

      if (originalVBlank & compatibilityVBlank & wideVBlank & wideCompatibilityVBlank) begin
        compatibilityTimingReg <= io_options_compatibility;
        wideTimingReg <= io_options_wideTiming;
      end

      timingClockEnableReg <= (wideTimingReg & compatibilityTimingReg) ? wideCompatibilityClockEnable :
        wideTimingReg ? wideClockEnable :
        compatibilityTimingReg ? compatibilityClockEnable : originalClockEnable;
      timingDisplayEnableReg <= (wideTimingReg & compatibilityTimingReg) ? wideCompatibilityDisplayEnable :
        wideTimingReg ? wideDisplayEnable :
        compatibilityTimingReg ? compatibilityDisplayEnable : originalDisplayEnable;
      timingPosXReg <= (wideTimingReg & compatibilityTimingReg) ? wideCompatibilityPosX :
        wideTimingReg ? widePosX :
        compatibilityTimingReg ? compatibilityPosX : originalPosX;
      timingPosYReg <= (wideTimingReg & compatibilityTimingReg) ? wideCompatibilityPosY :
        wideTimingReg ? widePosY :
        compatibilityTimingReg ? compatibilityPosY : originalPosY;
      timingHSyncReg <= (wideTimingReg & compatibilityTimingReg) ? wideCompatibilityHSync :
        wideTimingReg ? wideHSync :
        compatibilityTimingReg ? compatibilityHSync : originalHSync;
      timingVSyncReg <= (wideTimingReg & compatibilityTimingReg) ? wideCompatibilityVSync :
        wideTimingReg ? wideVSync :
        compatibilityTimingReg ? compatibilityVSync : originalVSync;
      timingHBlankReg <= (wideTimingReg & compatibilityTimingReg) ? wideCompatibilityHBlank :
        wideTimingReg ? wideHBlank :
        compatibilityTimingReg ? compatibilityHBlank : originalHBlank;
      timingVBlankReg <= (wideTimingReg & compatibilityTimingReg) ? wideCompatibilityVBlank :
        wideTimingReg ? wideVBlank :
  ...
  CaveVideoTiming #(
    ...
    .V_TOTAL (10'h106),
    .CE_DIV  (4)
  ) timing_wideCompatibilityVideoTiming (
    .clock                   (io_videoClock),
    .reset                   (io_videoReset),
    .io_display_x            (videoRegsSizeX),
    .io_display_y            (videoRegsSizeY),
    ...
    .io_offset_x             (wideOffsetX),
    .io_offset_y             (wideOffsetY),
    .io_timing_clockEnable   (wideCompatibilityClockEnable),
    .io_timing_displayEnable (wideCompatibilityDisplayEnable),
    .io_timing_pos_x         (wideCompatibilityPosX),
    .io_timing_pos_y         (wideCompatibilityPosY),
    .io_timing_hSync         (wideCompatibilityHSync),
    .io_timing_vSync         (wideCompatibilityVSync),
    .io_timing_hBlank        (wideCompatibilityHBlank),
    .io_timing_vBlank        (wideCompatibilityVBlank)
  );
